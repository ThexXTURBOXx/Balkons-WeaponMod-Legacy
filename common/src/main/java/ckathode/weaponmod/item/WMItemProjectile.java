...
public abstract class WMItemProjectile extends WMItem implements WMDispenserExtension {
    ...
    public static final WMItemProjectile BULLET_MUSKET_ITEM =
            WMItemBuilder.createWMItemProjectile(new WMDispenserExtension() {
                @NotNull
                @Override
                ...
                public void playSound(@NotNull Consumer<BlockSource> origFn, @NotNull BlockSource blockSource) {
                    blockSource.level().playSound(null, blockSource.pos(), SoundEvents.GENERIC_EXPLODE.value(),
                            SoundSource.NEUTRAL, 3.0f, 1.0f / (RANDOM.nextFloat() * 0.4f + 0.7f));
                    blockSource.level().playSound(null, blockSource.pos(), SoundEvents.LIGHTNING_BOLT_THUNDER,
                            SoundSource.NEUTRAL, 3.0f, 1.0f / (RANDOM.nextFloat() * 0.4f + 0.4f));
                }
    ...
    public static final WMItemProjectile MORTAR_SHELL_ITEM =
            WMItemBuilder.createWMItemProjectile(new WMDispenserExtension() {
                ...
                public void playSound(@NotNull Consumer<BlockSource> origFn, @NotNull BlockSource blockSource) {
                    blockSource.level().playSound(null, blockSource.pos(), SoundEvents.GENERIC_EXPLODE.value(),
                            SoundSource.NEUTRAL, 3.0f, 1.0f / (RANDOM.nextFloat() * 0.2f + 0.2f));
                }
