...
public class WMItem extends Item {
    ...
    @Nullable
    public static Pair<EquipmentSlot, Integer> findAnyItemSlot(Player player, TagKey<Item> tag) {
        return findAnyItemSlot(player, is -> isItemInTag(is, tag));
    }

    @Nullable
    public static Pair<EquipmentSlot, Integer> findAnyItemSlot(Player player, Collection<Item> items) {
        return findAnyItemSlot(player, is -> isItemInList(is, items));
    }

    @Nullable
    public static Pair<EquipmentSlot, Integer> findAnyItemSlot(Player player, Predicate<ItemStack> matcher) {
    ...
    public static boolean consumeAnyInventoryItem(Player player, Collection<Item> items) {
        Pair<EquipmentSlot, Integer> slot = findAnyItemSlot(player, items);
        if (slot == null) return false;
        ItemStack stack = slot.getFirst() == EquipmentSlot.MAINHAND ? player.getInventory().getItem(slot.getSecond()) :
                player.getItemBySlot(slot.getFirst());
        stack.shrink(1);
    ...
    public static boolean consumeAnyInventoryItem(Player player, Predicate<ItemStack> matcher) {
        Pair<EquipmentSlot, Integer> slot = findAnyItemSlot(player, matcher);
        if (slot == null) return false;
        ItemStack stack = slot.getFirst() == EquipmentSlot.MAINHAND ? player.getInventory().getItem(slot.getSecond()) :
                player.getItemBySlot(slot.getFirst());
        stack.shrink(1);
