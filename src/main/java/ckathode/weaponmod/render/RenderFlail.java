...
public class RenderFlail extends WMRenderer<EntityFlail> {
    ...
    public void doRender(@Nonnull EntityFlail entityflail, double d, double d1, double d2,
        ...
        EntityLivingBase shooter = entityflail.getShooter() instanceof EntityLivingBase
                ? (EntityLivingBase) entityflail.getShooter() : null;
        if (shooter != null && !renderOutlines) {
            int k = shooter.getPrimaryHand() == EnumHandSide.RIGHT ? 1 : -1;
