...
public class RenderJavelin extends WMRenderer<EntityJavelin> {
    @Override
    public void renderEntity(EntityJavelin entityjavelin, double d, double d1, double d2, float f, float f1) {
        if (!BalkonsWeaponMod.instance.modConfig.itemModelForEntity) {
            ...
            GL11.glRotatef(entityjavelin.prevRotationYaw + (entityjavelin.rotationYaw - entityjavelin.prevRotationYaw) * f1 - 90.0f, 0.0f, 1.0f, 0.0f);
            GL11.glRotatef(entityjavelin.prevRotationPitch + (entityjavelin.rotationPitch - entityjavelin.prevRotationPitch) * f1 - 45.0f, 0.0f, 0.0f, 1.0f);
            ...
            RenderWMItem.INSTANCE.render(entityjavelin, d, d1, d2, getStackToRender(entityjavelin));
            GL11.glDisable(EXTRescaleNormal.GL_RESCALE_NORMAL_EXT);
            GL11.glPopMatrix();
        }
    }
