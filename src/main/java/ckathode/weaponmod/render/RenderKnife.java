...
public class RenderKnife extends WMRenderer<EntityKnife> {
    @Override
    public void renderEntity(EntityKnife entityknife, double d, double d1, double d2, float f, float f1) {
        if (!BalkonsWeaponMod.instance.modConfig.itemModelForEntity) {
            ...
            GL11.glRotatef(entityknife.prevRotationYaw + (entityknife.rotationYaw - entityknife.prevRotationYaw) * f1 - 90.0f, 0.0f, 1.0f, 0.0f);
            GL11.glRotatef(entityknife.prevRotationPitch + (entityknife.rotationPitch - entityknife.prevRotationPitch) * f1 - 45.0f, 0.0f, 0.0f, 1.0f);
            ...
            RenderWMItem.INSTANCE.render(entityknife, d, d1, d2, getStackToRender(entityknife));
            GL11.glDisable(EXTRescaleNormal.GL_RESCALE_NORMAL_EXT);
            GL11.glPopMatrix();
        }
    }
